  ...
  <section class="component modify-scheme-info">
    <i-form :label-width="110" :model="model" :rules="rules" ref="form" inline>
      ...
      <i-form-item label="方案名称" prop="schemeName">
        <i-input v-model="model.schemeName" :maxlength="50"></i-input>
      </i-form-item>
      ...
      <i-form-item label="征信保护天数" prop="creditDays">
        <i-input-number v-model="model.creditDays" :min="0" :max="99999"></i-input-number>
      </i-form-item>
      <i-form-item label="抵押方式" prop="mortgageType" v-if="!_schemeType">
      ...
      <i-form-item label="逾期保护天数" prop="overdueDays">
        <i-input-number v-model="model.overdueDays" :min="0" :max="99999"></i-input-number>
      </i-form-item>
      ...
      <i-form-item label="期数" prop="periods">
        <i-input-number v-model="model.periods" :min="0" :max="99"></i-input-number>
      </i-form-item>
      <i-form-item label="融资最小金额" prop="moneyMin">
        <i-input-number v-model="model.moneyMin" :min="0" :max="99999999" :formatter="$filter.moneyFormat" :parser="$filter.moneyParse"></i-input-number>
      </i-form-item>
      ...
      <i-form-item label="融资最大金额" prop="moneyMax">
        <i-input-number v-model="model.moneyMax" :min="0" :max="99999999" :formatter="$filter.moneyFormat" :parser="$filter.moneyParse"></i-input-number>
      </i-form-item>
...
export default class ModifySchemeInfo extends Vue {
  ...
  // 方案类型为融资租赁时，不可选择抵押方式
  @Watch('model.schemeType', { immediate: true })
  onSchemeTypeChange() {
    if (this._schemeType) {
      this.model.mortgageType = '';
    }
  }

  get _schemeType() {
    return this.model.schemeType === 10049 ? true : false
  }
