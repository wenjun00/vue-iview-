<!--合同合规详情页-->
<template>
  <section class="component contract-compliance-detail">
    <div class="operate-buttons">
      <div>
        ...
        <i-button class="blueButton minWidth" @click="onCheckContractClick">查看合同</i-button>
      </div>
      <div v-if="rowData.orderStatus === this.$enum.OrderCurrentState.CONTRACT_COMPLIANCE">
        <i-button class="blueButton minWidth" @click="confirmMatching">确认合同合规</i-button>
      </div>
    </div>
...
export default class ContractComplianceDetail extends Vue {
  ...
  private confirmMatching() {
    this.basicOrderComplianceService.basicContractCompliance(this.model.orderId).subscribe(
      data => {
...
.component.contract-compliance-detail {
  .operate-buttons {
    display: flex;
