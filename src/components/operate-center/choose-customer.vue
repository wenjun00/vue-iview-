<template>
  <section class="component choose-customer">
    <data-form :model="model" @on-search="refreshData" hidden-date-search>
      <template slot="input">
        ...
        <i-form-item prop="phone" label="联系电话">
          <i-input placeholder="请输入客户联系电话" v-model="model.phone" :maxlength="11"></i-input>
        </i-form-item>
    ...
    <data-box :show-config-column="false" :height="400" :columns="columns" highlightRow :data="dataSet" ref="databox" :page="pageService" @on-page-change="refreshData"></data-box>
  </section>
</template>
...
import { customerInfo } from "~/interfaces/work.interface";
...
export default class ChooseCustomer extends Vue {
            ...
            h("i-button",
              {
                ...
                on: {
                  click: () => {
                    this.showCustomerInfo({ id: row.customerId, enabledEdit: true, isApply: true })
                  }
                }
              },
              "查看")
   ...
   */
  get selectedData() {
    return (this.$refs.databox as DataBox).currentRow
  }
   ...
   */
  refreshData() {
    if (this.type == "Asessment") {
      this.basicAssessmentCarPriceAnalysisService.queryCustomerByAssessment(this.model, this.pageService)
        .subscribe(
          data => {
            data.forEach(v => {
      ...
      this.basicCustomerCenterService.queryHistoryAndIntentCustomer(this.model, this.pageService)
        .subscribe(
          data => {
            data.forEach(v => {
              v._highlight = v.customerId === this.customerId
