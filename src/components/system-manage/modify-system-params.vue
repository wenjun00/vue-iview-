      ...
      <i-row>
        <i-col :span="24">
          <i-form-item label="参数值" prop="paramValue">
            <i-input v-model="sysParamModel.paramValue" :maxlength="50"></i-input>
          </i-form-item>
        </i-col>
      </i-row>
      <i-row>
        <i-col :span="24">
          <i-form-item label="是否启用" prop="paramStatus">
            <i-select v-model="sysParamModel.paramStatus" :disabled="!isEdit">
      ...
      <i-row>
        <i-col :span="24">
          <i-form-item label="说明" prop="remark">
            <i-input type="textarea" v-model="sysParamModel.remark" :disabled="!isEdit" :maxlength="100"></i-input>
          </i-form-item>
        </i-col>
      </i-row>
    </i-form>
...
export default class ModifySystemParams extends Vue {
  ...
  created() {}
  makeData(row) {
    // 如果参数为用户默认密码，则只允许修改参数值
    this.isEdit = row.paramCode === 'PersonalDefaultPassword' ? false : true
    ...
    this.sysParamModel.isReadonly = row.isReadonly;
  }
  /**
     * 确认修改
     */
