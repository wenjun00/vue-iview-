<!-- 审核 新车 -->
<template>
  <section class="component new-car">
    ...
    <i-collapse v-model="panel" accordion>
      ...
      <i-panel name="chooes-new-car">
        车辆信息
        <chooes-new-car slot="content" :isView="true" v-model="model.modelId" :orderId="orderId" ref="chooes-new-car"></chooes-new-car>
      </i-panel>
...
export default class NewCar extends Vue {
  ...
  private canNotViewStatus = {
    ...
    notLast: [...orderOpeater.first.status, ...orderOpeater.second.status, ...orderOpeater.submit.status]
  }
