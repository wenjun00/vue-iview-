    ...
    <div :style="getSubTreeStyle()">
      <div @click="onNodeClick" class="node-title" :class="{selected:selected}">
        <span @click="onExpand" v-if="!isLeaf" :class="{expanded:expanded}">
          <Icon v-show="loading" type="load-a" class="ivu-load-loop"></Icon>
          <Icon v-show="!loading" :type="expanded ? 'arrow-down-b' :'arrow-right-b'"></Icon>
        </span>
  ...
  onExpand() {
    if (!this.expanded) {
      if (this.root.loadData !== undefined && !this.data.loading) {
        // 将加载后的子数据给当前节点
        this.loading = true
        let load = this.root.loadData(this.data);
        if (load) {
