<template>
  <section class="component data-box">
    <div class="table">
      <i-table ref="table" :show-header="showHeader" class="i-table" :columns="columns" :data="data" stripe :highlight-row="highlightRow" :no-data-text="noDataText" :width="width" :height="height" @on-current-change="onCurrentChange" @on-selection-change="onSelectionChange" size="small"></i-table>
    </div>
...
export default class DataBox extends Vue {
  ...
  onDataChange() {
    ...
    this.mcurrentRow = this.data.find(r => r._highlight)
  }
  ...
  private mcurrentRow: any = null; // 多选数据
