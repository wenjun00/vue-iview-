      ...
      <div class="upload-materials">
        <i-dropdown trigger="click" slot="extra" @on-click="optionChange">
          ...
          <i-dropdown-menu slot="list">
            <i-dropdown-item v-for="{label, value} of $dict.getDictData(10085)" :key="value" :name="value">{{label}}</i-dropdown-item>
          </i-dropdown-menu>
        </i-dropdown>
      </div>
...
export default class MortgageOutStorage extends Vue {
  ...
  submit() {
    let data = {
      stockId: this.stockId,
      basicStockCarPhotoFile: this.dataSet,
      remark: this.remark
    }
...
<style lang="less">
.component.mortgage-out-storage {
  .upload-materials {
    text-align: right;
