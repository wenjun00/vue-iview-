<!-- 直租消费贷 二手车 -->
<template>
  <section class="component direct-rent-consume-order-old">
    ...
    <i-collapse v-model="panel" accordion>
      ...
      <i-panel name="chooes-old-car">
        车辆信息
        <chooes-old-car slot="content" :isView="isView" v-model="model.modelId" :orderId="orderId" ref="chooes-old-car"></chooes-old-car>
      </i-panel>
...
import ChooesOldCar from "./entry-manage-base/chooes-old-car.vue";
...
export default class DirectRentConsumeOrderOld extends Vue {
  ...
  mounted() {
    this.workComponents.push(this.$refs['chooes-old-car'] as ChooesOldCar)
  ...
  private async submit(data: any) {
    return await new Promise((resolve) => {
      this.basicCustomerOrderService.oldCarIncomingApply(data).subscribe(
        data => {
...
.component.direct-rent-consume-order-old {
  .operate-buttons {
    display: flex;
