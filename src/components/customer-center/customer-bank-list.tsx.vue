...
export default class CustomerBankList extends Vue {
  ...
  addBankCard() {
    this.$dialog.show({
      ...
      footer: true,
      width: 700,
      onOk: modifyCustomerInfoBank => {
        return modifyCustomerInfoBank.submit().then(v => {
  ...
  submit() {
    if (!this.currentCustomerBankRow) {
      this.$Message.info("请选择一条数据后再操作")
      return Promise.resolve(false);
    }
    return new Promise((resolve) => {
      this.basicCustomerAccountService
        .customerOpenAccount(this.currentCustomerBankRow, this.orderId)
        .subscribe(
          data => {
