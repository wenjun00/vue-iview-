<!--维护其他联系人信息-->
<template>
  <section class="component modify-customer-info-other-contacts">
    <i-form ref="form" inline :model="model" :rules="rules" :label-width="110">
      ...
      <i-form-item label="关系" prop="contactRelation">
        <i-select v-model="model.contactRelation" clearable>
          <i-option v-for="{value,label} in $dict.getDictData(10087)" :key="value" :label="label" :value="value"></i-option>
        </i-select>
      </i-form-item>
      <i-form-item label="联系方式" prop="contactPhone">
        <i-input v-model="model.contactPhone" :maxlength="11" clearable></i-input>
      </i-form-item>
      <i-form-item label="单位名称" prop="contractCompany">
        <i-input v-model="model.contractCompany" clearable :maxlength="50"></i-input>
      </i-form-item>
      <i-form-item label="职位" prop="contractJob">
        <i-input v-model="model.contractJob" clearable :maxlength="50"></i-input>
      </i-form-item>
      <i-form-item label="家庭地址" prop="area">
...
@Component({})
export default class ModifyCustomerInfoOtherContacts extends Vue {
  ...
  created() {
    this.model = {
      contactType: 10317, // 其他联系人
      contactName: '', // 姓名
      contactRelation: '', // 关系
      contactPhone: '', // 电话
      contractCompany: '', // 单位名称
      contractJob: '', // 职位
      province: '', // 所在省
      city: '', // 所在市
      area: '', // 所在区
