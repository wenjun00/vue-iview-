      ...
      <i-modal v-model="modifyPwdModal" title="修改密码">
        <i-form :label-width="110" ref="change-password" :model="repairModel">
          <i-form-item label="请输入原密码" prop="oldPassword">
            <i-input type="password" v-model="repairModel.oldPassword" :maxlength="20"></i-input>
          </i-form-item>
          <i-form-item label="请输入新密码" prop="newPasswordOne">
            <i-input type="password" v-model="repairModel.newPasswordOne" :maxlength="20"></i-input>
          </i-form-item>
          <i-form-item label="确认新密码" prop="newPassword">
            <i-input type="password" v-model="repairModel.newPassword" :maxlength="20"></i-input>
          </i-form-item>
        </i-form>
...
export default class WorkHeader extends Vue {
   ...
   * 修改密码后重新登录
   */
  updatePassWordLoginOut() {
    this.$Modal.confirm({
      title: '提示',
      content: '密码已修改，是否重新登录',
      onOk: () => {
        this.$router.push('/')
      }
    })
