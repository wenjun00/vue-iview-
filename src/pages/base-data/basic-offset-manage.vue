  ...
  <section class="page basic-offset">
    <page-header title="冲抵项管理" hiddenPrint hiddenExport>
      <div v-if="checkedBasicOffsetId">
        <command-button class="command-add" label="取消发布" v-auth="591"  @click="cancelRelease" v-show="isPublish"></command-button>
        <command-button class="command-add" label="发布" v-auth="590"  @click="publish" v-show="!isPublish"></command-button>
      </div>
      <command-button v-show="!isPublish" class="command-add" label="新增冲抵项" v-auth="589" @click="basicOffsetItemOperate(checkedBasicOffsetId,expenseDataSet)"></command-button>
    </page-header>
      ...
      <i-row type="flex" style="flex:1;">
        <i-col :span="4" style="padding:10px 0 10px 0;display:flex;flex-direction: column;">
          <div class="data-form-item">
            ...
            <span>冲抵策略</span>
            <span @click="basicOffsetOperate()" v-auth="619" class="data-form-item-add">
