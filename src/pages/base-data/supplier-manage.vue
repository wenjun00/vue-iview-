<template>
  <section class="page supplier-manage">
    <page-header title="供应商管理" hidden-print hidden-export>
      <command-button label="新增供应商" @click="modifySupplier()" v-auth="582"></command-button>
    </page-header>
    <data-form :model="model" @on-search="refreshData" hidden-date-search>
      <template slot="input">
        <i-form-item prop="name" label="供应商：">
          <i-input placeholder="请输入供应商名称" v-model="model.name" :maxlength="50"></i-input>
        </i-form-item>
      </template>
    </data-form>
          ...
          return h("div", [
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(583),
                class: {
            ...
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(584),
                style: {
                  color: "#265EA2"
                },
                on: {
