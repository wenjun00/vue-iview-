<!--直租消费贷车辆匹配-->
<template>
  <section class="page direct-rent-consume-matching">
    <page-header title="直租消费贷" hidden-print hidden-export></page-header>
...
import { OrderCurrentState,BusinessType } from "~/config/enum.config"
...
export default class DirectRentConsumeMatching extends Page {
    ...
    this.columns = [
      {
        ...
        render: (h, { row, column, index }) => {
          return (<div>
            <i-button type="text" class="row-command-button" v-auth={551} onClick={() => this.checkOrderInfo(row)}>{row.orderStatus === this.$enum.OrderCurrentState.MATCH ? '车辆匹配' : '查看'}</i-button>
          </div>)
        }
      },
      {
   ...
   */
  refreshData() {
    this.basicOrderComplianceService.queryOrderDirectRentList(this.model, this.pageService).subscribe(
    ...
    this.$dialog.show({
      ...
      render: h => h(MatchingCarDetail, {
        props: {
          ...
          businessType: BusinessType.CONSUME_DIRECT
        }
      })
    })
  }
