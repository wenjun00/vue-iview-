  ...
  created() {
    this.columns = [
      ...
      },
      {
        ...
        render: (h, { row }) => (<i-button type="text" class="row-command-button" onClick={() => this.showOrderInfo({id: row.orderId, orderType: row.orderType})}>{row.orderNo}</i-button>)
      },
      {
        align: 'center',
        title: '客户姓名',
      ...
      },
      {
        ...
        key: 'financeTime',
        minWidth: this.$common.getColumnWidth(4),
        render: (h, { row }) => (<span>{this.$filter.dateTimeFormat(row.financeTime)}</span>)
      },
    ]
  }
