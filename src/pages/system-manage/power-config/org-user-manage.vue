...
<template>
  <section class="page org-user-manage">
    <page-header title="机构与用户管理" hidden-print hidden-export>
      <i-button class="blueButton" @click="addNewUser" v-auth="553">新增用户</i-button>
      <i-button class="blueButton" @click="batchAllotRole" v-auth="554">批量分配角色</i-button>
      <i-button class="blueButton" @click="batchManageDevice" v-auth="555">批量管理设备</i-button>
      <i-button class="blueButton" @click="addDept" v-auth="556">添加机构</i-button>
    </page-header>
    <div class="org-user-manage-container">
      <div class="org-tree container-height">
      ...
      <div class="user-list">
        <data-form ref="user-search-form" hidden-date-search :model="userSearchModel" @on-search="searchUserByCondition" :page="pageService">
          <template slot="input">
            <i-form-item prop="userName" label="用户名：">
              <i-input v-model="userSearchModel.userName" placeholder="请输入用户名" :maxlength="20"></i-input>
            </i-form-item>
            <i-form-item prop="realName" label="姓名：">
              <i-input v-model="userSearchModel.realName" placeholder="请输入姓名" :maxlength="20"></i-input>
            </i-form-item>
        ...
        <data-box :columns="columns1" :data="userList" ref="databox" @on-page-change="searchUserByCondition" :page="pageService" @on-selection-change="onSelectionChange"></data-box>
      </div>
    </div>
  </section>
</template>
...
import ModifyUserRole from "~/components/system-manage/modify-user-role.vue";
...
export default class OrgUserManage extends Page {
  ...
  created() {
    ...
    this.columns1 = [
      ...
      {
        title: "操作",
        width: this.$common.getOperateWidth(4),
        fixed: "left",
        align: "center",
          ...
          h("div", [
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(557),
                style: {
                  color: "#265EA2"
                },
                on: {
            ...
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(558),
                style: {
                  color: "#265EA2"
                },
                on: {
            ...
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(559),
                style: {
                  color: "#265EA2"
                },
                on: {
            ...
            h(
              "i-button",
              {
                ...
                directives: this.$common.bindAuth(560),
                style: {
                  color: "#265EA2"
                },
                on: {
      ...
      },
      {
        ...
        minWidth: this.$common.getColumnWidth(4),
        render: (h, { row }) =>
          h("span", {}, this.$filter.dictConvert(row.userStatus))
      },
      {
        align: "center",
        editable: true,
      ...
      },
      {
        ...
        render: (h, { row, columns, index }) => {
          return h("span", this.$filter.dateTimeFormat(row.operatorTime));
        }
      },
      {
        align: "center",
  ...
  private searchUserByCondition() {
    ...
    this.sysUserService
      .findUserByOrgAuth(this.userSearchModel, this.pageService)
      .subscribe(
  ...
  private removeDept() {
    this.$Modal.confirm({
      ...
      content: "确定删除此组织机构吗？",
      onOk: () => {
        this.sysOrgService.deleteOrganization(this.selectOrg.id).subscribe(
          data => {
      ...
      render: h =>
        h(ModifyOrg, {
          props: {
            ...
            orgPname: this.selectOrg.orgName
          }
        })
    });
  }
