<template>
  <section class="page contract-compliance-check">
    <page-header title="合同合规" hidden-print hidden-export></page-header>
...
import ContractComplianceDetail from "~/components/compliance-check/contract-compliance-detail.tsx.vue"
...
export default class ContractComplianceCheck extends Page {
    ...
    this.columns = [
      {
        ...
        render: (h, { row, column, index }) => {
          return (<div>
            <i-button type="text" class="row-command-button" v-auth={545} onClick={() => this.checkOrderInfo(row)}>{row.orderStatus === this.$enum.OrderCurrentState.CONTRACT_COMPLIANCE ? '合同合规' : '查看'}</i-button>
          </div>)
        }
      },
      {
