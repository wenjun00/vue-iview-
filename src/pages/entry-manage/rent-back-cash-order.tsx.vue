<!--回租现金贷订单-->
<template>
  <section class="page rent-back-cash-order">
    <page-header title="回租现金贷订单" hiddenPrint hiddenExport>
      <command-button label="二手车进件申请" v-auth="616" @click="() => this.oldCarApplyClick(null)"></command-button>
    </page-header>
...
export default class RentBackCashOrder extends Page {
  ...
  private businessType = BusinessType.CASH_BACK;
    ...
    this.columns = [
      {
        ...
        render: (h, { row, column, index }) => {
          ...
          if (orderOpeater.submit.status.includes(row.orderStatus)) {
            // buttons.push(<i-button type="text" class="row-command-button" v-auth={617} onClick={() => this.onDeleteClick(row.id)}>删除</i-button>)
            buttons.push(<i-button type="text" class="row-command-button" v-auth={618} onClick={() => this.onEditClick(row.id, row.orderCarType)}>编辑</i-button>)
          }
  ...
  // 刷新页面数据
  private refreshData() {
    this.basicCustomerOrderService.queryOrderCashReturnPage(this.model, this.pageService).subscribe(
