<!--回租消费贷订单-->
<template>
  <section class="page rent-back-consume-order">
    <page-header title="回租消费贷订单" hiddenPrint hiddenExport>
      <command-button label="新车进件申请" v-auth="612" @click="() => this.newCarApplyClick(null)"></command-button>
      <command-button label="二手车进件申请" v-auth="613" @click="() => this.oldCarApplyClick(null)"></command-button>
    </page-header>
...
export default class RentBackConsumeOrder extends Page {
  ...
  private businessType = BusinessType.CONSUME_BACK;
    ...
    this.columns = [
      {
        ...
        render: (h, { row, column, index }) => {
          ...
          if (orderOpeater.submit.status.includes(row.orderStatus)) {
            // buttons.push(<i-button type="text" class="row-command-button" v-auth={614} onClick={() => this.onDeleteClick(row.id)}>删除</i-button>)
            buttons.push(<i-button type="text" class="row-command-button" v-auth={615} onClick={() => this.onEditClick(row.id, row.orderCarType)}>编辑</i-button>)
          }
  ...
  // 刷新页面数据
  private refreshData() {
    this.basicCustomerOrderService.queryOrderConsumeReturnPage(this.model, this.pageService).subscribe(
