<template>
  <section class="page rent-back-consume-order-approval">
    <page-header title="回租消费贷" hiddenPrint hiddenExport>
    </page-header>
...
export default class RentBackConsumeOrderApproval extends Page {
    ...
    this.columns = [
      {
        ...
        render: (h, { row, column, index }) => {
          return <i-button type="text" class="row-command-button" v-auth={548} onClick={() => this.onApprovalClick(row.id, row.orderCarType, row.businessType)}>审核</i-button>
        }
      },
      {
        align: "center",
