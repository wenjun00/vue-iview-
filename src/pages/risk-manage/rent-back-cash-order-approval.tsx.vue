<template>
  <section class="page rent-back-cash-order-approval">
    <page-header title="回租现金贷" hiddenPrint hiddenExport>
    </page-header>
...
export default class RentBackCashOrderApproval extends Page {
  ...
  this.columns = [
    {
      ...
      render: (h, { row, column, index }) => {
        return <i-button type="text" class="row-command-button" v-auth={549} onClick={() => this.onApprovalClick(row.id, row.orderCarType, row.businessType)}>审核</i-button>
      }
    },
    {
      align: "center",
